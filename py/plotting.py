...
def LHMR_savederr(f: GroupCatalog, show_all=False, inset: GroupCatalog=None, savedata=False):
    ...
    log_mean_all[np.isnan(yerr_all_lower) | np.isnan(yerr_all_upper)] = np.nan
    log_means_r[np.isnan(yerr_r_lower) | np.isnan(yerr_r_upper)] = np.nan
    log_means_b[np.isnan(yerr_b_lower) | np.isnan(yerr_b_upper)] = np.nan
    ...
    # Make sure the shaded regions are no longer than what we will show in data.
    b_valid = ~np.isnan(log_means_b)
    r_valid = ~np.isnan(log_means_r)
    syserr_b_upper = np.where(b_valid, syserr_b_upper, np.nan)
    syserr_b_lower = np.where(b_valid, syserr_b_lower, np.nan)
    syserr_r_upper = np.where(r_valid, syserr_r_upper, np.nan)
    syserr_r_lower = np.where(r_valid, syserr_r_lower, np.nan)
    if show_all:
        all_valid = ~np.isnan(log_mean_all)
        syserr_all_upper = np.where(all_valid, syserr_all_upper, np.nan)
        syserr_all_lower = np.where(all_valid, syserr_all_lower, np.nan)
    plt.figure(dpi=DPI)
    x_vals = np.log10(Mhalo_labels)
    x_vals2 = np.log10(Mhalo_labels2)
    ...
    if inset is not None:
        ...
        ax_inset.plot(x_vals, np.log10(ratio), '-', color='k', label='BGS')
        ax_inset.plot(x_vals2[:-6], np.log10(ratio2[:-6]), '-', color='purple', label='SDSS')
        ax_inset.set_xlim(10,15)
        ax_inset.legend(fontsize=8)
...
# Tabulated log-normal SHMR scatter (sigma_log(M*)) vs halo mass from Shao+2026.
# Columns: log(Mh / [Msun/h]), sigma_50 (median), sigma_16, sigma_84
_SHAO_SHMR_SCATTER_TABLE = np.array([
    [10.0, 0.8401126912996586, 0.410846336291615, 1.0],
    [10.05, 0.8238973730884183, 0.40543336258281043, 1.0],
    [10.1, 0.8076192640301307, 0.3991357173146875, 1.0],
    [10.15, 0.7909898508662824, 0.3932653154431311, 1.0],
    [10.2, 0.7747376718404657, 0.38756901265933014, 1.0],
    [10.25, 0.7585401995151053, 0.38184316500207593, 1.0],
    [10.3, 0.7425174244325762, 0.37600023930075543, 0.9927786316927024],
    [10.35, 0.7261208649300083, 0.37000173596870645, 0.9698217999967482],
    [10.4, 0.7099425265631834, 0.36418887568308506, 0.9465589499676266],
    [10.45, 0.6935770907667017, 0.35809175128956, 0.9229108314574371],
    [10.5, 0.6770781708545417, 0.3522352120407389, 0.8985756324246419],
    [10.55, 0.6600311363556658, 0.3461268411294775, 0.8749819014581465],
    [10.6, 0.6439987076470015, 0.3402911102915234, 0.8523427338999813],
    [10.65, 0.6279761419991323, 0.33430471657721955, 0.8287453651223797],
    [10.7, 0.6109011333458937, 0.32832788475604663, 0.8053536066007967],
    [10.75, 0.5942162645720649, 0.3221588245151476, 0.7815839877441274],
    [10.8, 0.577474593576998, 0.31620724416783974, 0.7577832929772629],
    [10.85, 0.5605847292836466, 0.3098056498391147, 0.7343950859320442],
    [10.9, 0.5439401594799578, 0.303862222718262, 0.710565018636572],
    [10.95, 0.527654003783592, 0.29815833536397257, 0.6867463584694339],
    [11.0, 0.5110544569465045, 0.29280518784418813, 0.6631217106173048],
    [11.05, 0.4943057206127474, 0.286914315885608, 0.6393537515469743],
    [11.1, 0.4779815832576487, 0.28049919823507596, 0.6159513735788531],
    [11.15, 0.4611725352576268, 0.2741263670638316, 0.5919765503282075],
    [11.2, 0.44372914647324835, 0.26830870988733935, 0.5686594925303824],
    [11.25, 0.4277102256340594, 0.2619724635362914, 0.5447886055980167],
    [11.3, 0.41086126587184213, 0.2559670637130603, 0.5215955650681099],
    [11.35, 0.39336839900411846, 0.2500202773658074, 0.4979864015925634],
    [11.4, 0.37703296630790595, 0.24423320913278357, 0.4752546311540553],
    [11.45, 0.3604600502207197, 0.23812061204958632, 0.45213588560395546],
    [11.5, 0.34348796271204163, 0.23222728949171814, 0.42938714824316715],
    [11.55, 0.32671626116438757, 0.2258970993377119, 0.4073349208678689],
    [11.6, 0.30948359890544386, 0.2189343705977452, 0.3853337653390758],
    [11.65, 0.29188238369236136, 0.21223088539387816, 0.3631935689902465],
    [11.7, 0.27385411198641924, 0.20545261884518598, 0.3420988401130786],
    [11.75, 0.2567654884988729, 0.19847914155503285, 0.3212637758599869],
    [11.8, 0.23985898603155942, 0.1904566961203086, 0.30060770963814654],
    [11.85, 0.22345088219613943, 0.18427711776835587, 0.2792331872972141],
    [11.9, 0.20861426934234992, 0.17771683100888883, 0.25914996092248505],
    [11.95, 0.19702158391759167, 0.1723516968505973, 0.23944036951497305],
    [12.0, 0.18817000944060647, 0.1677895465769295, 0.22121964129945518],
    [12.05, 0.18236489532376757, 0.163887921812572, 0.20622070353797714],
    [12.1, 0.17832174513826432, 0.16100441507965155, 0.1985257463969541],
    [12.15, 0.17592587033935714, 0.15913299716580798, 0.19351790862568763],
    [12.2, 0.17442434626351666, 0.15796174671156504, 0.19084027351497582],
    [12.25, 0.1735604884933569, 0.15696500541677746, 0.18969664169596678],
    [12.3, 0.1731939124907782, 0.1566607043225612, 0.18908706965223215],
    [12.35, 0.17303805235962605, 0.1564802705758135, 0.1888188006946053],
    [12.4, 0.1729392638320906, 0.15624249635695434, 0.18877852634922895],
    [12.45, 0.17292756185498348, 0.15610820008196963, 0.18877852634922895],
    [12.5, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.55, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.6, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.65, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.7, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.75, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.8, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.85, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.9, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [12.95, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.0, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.05, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.1, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.15, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.2, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.25, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.3, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.35, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.4, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.45, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.5, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.55, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.6, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.65, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.7, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.75, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.8, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.85, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.9, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [13.95, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.0, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.05, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.100000000000001, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.15, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.2, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.25, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.3, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.350000000000001, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.4, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.45, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.5, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.55, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.600000000000001, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.65, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.7, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.75, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.8, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.850000000000001, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.9, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [14.95, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
    [15.0, 0.17292756185498348, 0.15608387661550172, 0.18877852634922895],
])

def shao_shmr_scatter_eq(logMh):
    """
    Interpolate the tabulated Shao+2026 log-normal SHMR scatter (sigma_log(M*))
    as a function of halo mass (H=100h) onto the requested logMh values.
    Returns (median, 16th percentile, 84th percentile) arrays matching the
    shape of logMh.
    """
    logMh = np.asarray(logMh)
    table_logMh = _SHAO_SHMR_SCATTER_TABLE[:, 0]
    sigma_50 = _SHAO_SHMR_SCATTER_TABLE[:, 1]
    sigma_16 = _SHAO_SHMR_SCATTER_TABLE[:, 2]
    sigma_84 = _SHAO_SHMR_SCATTER_TABLE[:, 3]

    shao_best = np.interp(logMh, table_logMh, sigma_50)
    lo = np.interp(logMh, table_logMh, sigma_16)
    hi = np.interp(logMh, table_logMh, sigma_84)
...
def SHMR_scatter_litcompare(f: GroupCatalog, sdss: GroupCatalog, savedata=False):
    ...
    # add ticks to both side of axes. major and minor
    plt.tick_params(axis='x', which='both', top=True, bottom=True)
    plt.tick_params(axis='y', which='both', right=True, left=True)
...
def fsat_with_err_from_saved(gc: GroupCatalog, show_all=False, savedata=False):
    ...
    if show_all:
        axes[0].errorbar(LogLgal_labels, fsat, yerr=gc.fsat_bootstrap_err, fmt='o', label='All', markerfacecolor='k', markersize=5, markeredgecolor='k', markeredgewidth=1.5, elinewidth=2, capsize=5, ecolor='k')
    ...
    axes[0].legend()
    axes[0].set_xlim(6.85, LOG_LGAL_MAX_TIGHT)
...
def fsat_incompleteness_study_all(catalogs, truthcat: GroupCatalog):
   ...
   # axes.errorbar(obs_lcen, np.log10(obs_ratio), yerr=obs_ratio_err_log, fmt='o', markerfacecolor='white', markeredgecolor='k', markeredgewidth=2, markersize=6, capsize=3, elinewidth=2, ecolor='k', label='Data')
    ...
    plt.errorbar(truthcat.L_gal_labels, truthcat.f_sat, yerr=truthcat.fsat_bootstrap_err, label=get_dataset_display_name(truthcat), fmt='o', markerfacecolor='white', markeredgecolor=truthcat.color, markeredgewidth=1.5, markersize=4, capsize=4, elinewidth=2, ecolor=truthcat.color)
    for f in catalogs:
        plt.plot(f.L_gal_labels, f.f_sat, f.marker, label=get_dataset_display_name(f), color=f.color, linewidth=2)
...
def compare_wp_rp_caldata(c1: CalibrationData, c2: CalibrationData, savedata=False):
    ...
    fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(2.5 + 3 * ncols, 3.8 * nrows), dpi=DPI)
...
def gfparams_plots(gc: GroupCatalog, chains_flat, savedata=False):
    ...
    # Go from the 10 parameters to the desired functions of them, evaluated at x
    w_samples, bsat_q_samples, bsat_sf_samples = chains_to_wcen_bsat(chains_flat, x)
    ...
    w_lower2 = np.percentile(w_samples, 2.5, axis=0)
    w_upper2 = np.percentile(w_samples, 97.5, axis=0)
    bsat_q_lower2 = np.percentile(bsat_q_samples, 2.5, axis=0)
    bsat_q_upper2 = np.percentile(bsat_q_samples, 97.5, axis=0)
    bsat_sf_lower2 = np.percentile(bsat_sf_samples, 2.5, axis=0)
    bsat_sf_upper2 = np.percentile(bsat_sf_samples, 97.5, axis=0)
                             
    
    # Plot shaded regions
    axes[0].fill_between(x, w_lower, w_upper, color='purple', alpha=0.5, label='68% CI')
    axes[0].fill_between(x, w_lower2, w_upper2, color='purple', alpha=SHADED_ERR_ALPHA, label='95% CI')
    axes[1].fill_between(x, bsat_q_lower, bsat_q_upper, color='red', alpha=0.5)
    axes[1].fill_between(x, bsat_q_lower2, bsat_q_upper2, color='red', alpha=SHADED_ERR_ALPHA)
    axes[1].fill_between(x, bsat_sf_lower, bsat_sf_upper, color='blue', alpha=0.5)
    axes[1].fill_between(x, bsat_sf_lower2, bsat_sf_upper2, color='blue', alpha=SHADED_ERR_ALPHA)
...
def sdss_bgs_mag_compare_plot(bgs: GroupCatalog, sdss: GroupCatalog, showpoly=True, showsaved=True, quiescent=None, savedata=False):
    ...
    # Generate fitted values
    x_fit = np.linspace(-23.5, x.max() + 2, 100)
    y_fit = Polynomial(coeffs)(x_fit)

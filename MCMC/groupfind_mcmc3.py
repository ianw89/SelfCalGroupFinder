...
def chisqr(params):
    ...
    #print(args)
    #sp.run(args, cwd='/home/users/imw2293/SelfCalGroupFinder/bin/', stdout=f)

    global ncount
    ncount = ncount + 1

    # Make command to call group finder
    os.system('mv outxx outyy')
    cmd = '/home/users/imw2293/SelfCalGroupFinder/bin/kdGroupFinder_omp '+galdata_file+'  0 1 0.179 1 1 '
    cmd = cmd+' '+str(params[0])+' '+str(params[1])+' '+str(params[2])+' '+str(params[3])
...
nwalkers = 30
niter = 40000
initial = np.array([   11.58410121 ,  0.97362779, \
  10.52856046 ,  0.97052776 ,  2.1941562 ,  -0.99936404 ,  5.00093328, \
   2.61706251,  12.8518478 ,  -4.02863109, 1.93342061 ,  1.63162082 , -0.1, -0.79929147  ])
   
# new bestfit from the MY_MCMC runs
...
ndim = len(initial)
p0 = [np.array(initial)*(1+ 0.002 * np.random.randn(ndim)) for i in range(nwalkers)]


# -- set up the backend
fname='mcmc_'+"{:02}".format(int(sys.argv[2]))+'.dat'
